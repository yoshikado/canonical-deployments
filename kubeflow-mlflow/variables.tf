...
variable "kubeflow_mlflow_config" {
  description = "configuration for the kubeflow-mlflow deployment"
  type = object({
    ...
    public_url                            = optional(string, "http://dex-auth.kubeflow.svc:5556")
    enable_mlflow_nodeport                = optional(bool, true)
    mlflow_nodeport                       = optional(number, 31380)
    mlflow_dashboard_link                 = optional(bool, true)
    mlflow_kserve_integration             = optional(bool, true)
    mlflow_minio_size                     = optional(string, "10G")
    mlflow_mysql_size                     = optional(string, "10G")
  })
}
